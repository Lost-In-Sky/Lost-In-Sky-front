...
function CalendarComponent() {
  ...
  const isDateDisabled = (date) => {
    return disabledDates.some(
      (disabledDate) => disabledDate.getTime() === date.date.getTime()
    );
  };
  ...
  return (
    <div>
      <Calendar
        ...
        locale={hy.locale}
        formatShortWeekday={(locale, value) => hy.weekdaysShort[value.getDay()]}
        formatMonthYear={(locale, value) =>
          `${hy.months[value.getMonth()]} ${value.getFullYear()}`
        }
        formatWeekday={(locale, value) => hy.weekdaysLong[value.getDay()]}
      />
    </div>
  );
}
